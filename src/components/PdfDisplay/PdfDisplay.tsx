  ...
  const allTrips = data.trips?.map((trip) => {
    return (
      ...
      <View key={trip.id} style={styles.section}>
        ...
        <Text style={styles.text}>Localisation du voyage : {trip.localisation}</Text>
        <Text style={styles.text}>Description du voyage : {trip.description}</Text>
        <Text style={styles.text}>Url de l&apos;image du voyage : {trip.url_image}</Text>
        <Text style={styles.text}>Message alternatif à l&apo;image : {trip.alt_image}</Text>
      </View>
    );
  });
  const allLinks = data.links?.map((link) => {
    return (
      <View key={link.id} style={styles.section}>
        <Text style={styles.text}>Identifiant de la proposition : {link.id}</Text>
        <Text style={styles.text}>Description de la proposition : {link.description}</Text>
        <Text style={styles.text}>URL partagée: {link.url}</Text>
        <Text style={styles.text}>Localisation de la proposition : {link.localisation}</Text>
        <Text style={styles.text}>Titre de la proposition : {link.title}</Text>
        <Text style={styles.text}>Url de l&apos;image de la proposition : {link.image}</Text>
        <Text style={styles.text}>Message alternatif à l&apo;image : {link.alt_image}</Text>
      </View>
    );
  });
